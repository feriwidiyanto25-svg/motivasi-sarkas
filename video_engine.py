...
def calculate_reading_duration(text, min_duration, max_duration):
    ...
    # Ditambah 1 detik sebagai jeda napas/waktu mencerna bacaan
...
# ==========================================
# PEXELS SEARCH (ORIGINAL RESTORED)
# ==========================================
...
# ==========================================
# CHOOSE VIDEO FILE (ORIGINAL RESTORED)
# ==========================================
...
# ==========================================
# FETCH BACKGROUND (ORIGINAL RESTORED)
# ==========================================
def fetch_background_video(naskah, target_duration):
    ...
    print(f"Video score: {best['score']:.2f}")
    ...
    for candidate_index, candidate in enumerate(ranked[:4], start=1):
        ...
        try:
            ...
            if len(content) < 10000:
                print(f"File background terlalu kecil: {len(content)} bytes.")
                continue

            with open(output_path, "wb") as file:
                file.write(content)
            
            print(f"Background tersimpan: {output_path}")
            ...
            try:
                ...
                print(f"Background valid: {test_clip.w}x{test_clip.h}, {test_clip.duration:.2f}s")
                return output_path
            ...
            print(f"Download Pexels error: {e}")
        except Exception as e:
            print(f"Background error: {e}")

    print("Semua kandidat background gagal di-download atau divalidasi.")
    return None
...
def fit_video_to_vertical(video):
    ...
    print(f"Background asli: {current_width}x{current_height}")
    ...
    print(f"Background final: {video.w}x{video.h}")
    return video

# ==========================================
# TEXT CREATION (UPDATED WITH POPPINS)
# ==========================================
def create_text_clip(text, fontsize, color, start, duration, stroke_width):
    # Membaca font langsung dari folder assets sesuai screenshot
    font_path = os.path.abspath(os.path.join("assets", "Poppins-Bold.ttf"))
    
    # Fallback jika ternyata font belum dipull/tidak ada
    if not os.path.exists(font_path):
        print(f"⚠️ Peringatan: Font tidak ditemukan di {font_path}. Menggunakan font cadangan.")
...
def generate_text_overlay(naskah, timings):
    ...
    # Simpan dalam format tuple seperti segment specs di fungsi utama
...
def create_audio(timings):
    try:
        ...
        if not setup_pool:
            print("Audio setup tidak tersedia.")
            return None
...
def render_final_video(naskah):
    ...
    try:
        ...
        if bg_path and os.path.exists(bg_path):
            ...
            # Jika video Pexels kurang panjang dari durasi narasi, kita loop agar tidak hitam
        ...
        video = video.fx(vfx.colorx, 0.4) # Gelapkan BG

        text_segments_info = generate_text_overlay(naskah, timings)
        composed_segments = []

        # MEMORY OPTIMIZED COMPOSITION (Berdasarkan kode originalmu)
        for segment_name, text_clip, start, duration in text_segments_info:
            ...
            segment = (
                CompositeVideoClip([segment_bg, segment_text], size=(VIDEO_WIDTH, VIDEO_HEIGHT))
                .set_duration(duration)
            )

            # KODE ORIGINAL GENERATE THUMBNAIL DIKEMBALIKAN
            if segment_name == "title":
                ...
                try:
                    ...
                    print(f"Thumbnail tersimpan: {thumbnail_path} ({thumbnail_size / 1024:.1f} KB)")
                except Exception as thumbnail_error:
                    print(f"⚠️ Gagal membuat thumbnail: {thumbnail_error}")
                    try:
            ...
            # Simpan reference clip untuk dicleanup nanti
        ...
        # NAMA FILE OUTPUT DIKEMBALIKAN KE 'final_'
        ...
        if bg_path and os.path.exists(bg_path):
            try:
                os.remove(bg_path)
                print(f"Temporary background dihapus: {bg_path}")
            except: pass
        gc.collect()
